...
const Chart = (props: { width: number; height: number; color: string; data: number[]; }) => {
  ...
  /*const drawChart = (width: number, height: number, color: string, data: { country: string, value: number}[]) => {
    ...
    //const highestValue = Math.ceil(highest / 1000) * 1000;
    ...
    const svg = select(ref.current)
      .append('svg')
      .attr('width', width + margins.left + margins.right + margins.left * 4)
    ...
    svg
      ...
      .attr('x', (d) => x(d.country) as number + margins.left * 4)
      .attr('y', (_d) => y(0));
    // Bar value-labes
    svg
      .append('g')
      ...
      .classed('label', true)
      .text((d) => d.value)
      .attr('x', (d) => x(d.country) as number + x.bandwidth() / 2  + margins.left * 4)
      .attr('y', (_d) => y(0) - 10)
    ...
    svg
      ...
      .attr('transform', `translate(${margins.left * 4}, ${height})`)
      .attr('color', color);
    // y-Axis labels
    svg
      .append('g')
      .call(axisLeft(y).tickSizeOuter(0))
      .attr('transform', `translate(${margins.left * 4 + 1}, 0)`)
      .attr('color', color);
    // Animation Bars
    svg
      .selectAll('rect')
